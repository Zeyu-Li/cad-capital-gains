...
def capgains_calc(transactions, year, tickers=None, output_format='table'):
    """Take a list of txs and output the calculated capital gains.
    ...
    if output_format == 'json':
        ...
        for ticker in filtered_transactions.tickers:
            transactions_to_report = calculate_gains(
                filtered_transactions, year, ticker
            )
            if not transactions_to_report:
                results[ticker] = {
                    'year': year, 'total_gains': 0, 'transactions': []
                }
                continue
            ...
            results[ticker] = {
                ...
                'transactions': [
                    _transaction_to_dict(t) for t in transactions_to_report
                ]
            }
